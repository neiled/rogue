...
{
	return (float) width*(x)/(maxx-minx);
}
...
void circle(float x, float y, float r) {
	cairo_set_source_rgb (cr, 0.0, 0.0, 0.0);
	cairo_move_to(cr, X(x), Y(y));
	cairo_arc (cr, X(x), Y(y), D(r), 0, 2*M_PI);
	cairo_stroke(cr);
}
...
int main(int argc, char *argv[]) 
{
  ...
  cr = cairo_create (surface);
	...
	float beta = param;
	float alpha = acos(1 - 2/(beta*beta));
	float theta = (M_PI - alpha)/2;
	float R = Geometry<float>::distanceL2(p,q);
	float x = 1/beta;// R*sin(alpha/2);
	
	line(x,-1, x, 1);
	
	float Rp = 0.5*(R + R/(beta-1));
	circle(q[0]-Rp,0,Rp);
