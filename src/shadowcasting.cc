...
//    _starty = starty;
    ...
    for (int i = -1; i < 2; i+=2)
    {
      for (int j = -1; j < 2; j+=2)
      {
        ...
        castLight(1, 1.0f, 0.0f, startx, starty, 0, i, j, 0, resistanceMap);
        castLight(1, 1.0f, 0.0f, startx, starty, i, 0, 0, j, resistanceMap);
      }
...
void ShadowCasting::castLight(int row, float start, float end, int startx, int starty, int xx, int xy, int yx, int yy, resistance_map_t resistance_map) {
    float newStart = 0.0f;
    if (start < end) {
        return;
    }
        ...
        for (int deltaX = -distance; deltaX <= 0; deltaX++) {
            ...
            if (blocked) { //previous cell was a blocking one
                ...
                if (blockingCell(resistance_map[currentY][currentX]) && distance < _radius) {//hit a wall within sight line
                    blocked = true;
                    castLight(distance + 1, start, leftSlope, startx, starty, xx, xy, yx, yy, resistance_map);
                    newStart = rightSlope;
                }
            }
        }
